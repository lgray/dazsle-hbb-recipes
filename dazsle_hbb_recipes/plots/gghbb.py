...
h_msd_ak8_topR6 = hist.Hist("Events / bin",
                            hist.Cat("dataset","Dataset"),
                            hist.Cat("passfail","Pass or Fail selection"),
                            hist.Cat("systematic","Systematic Variations"),
                            hist.Bin("msd",r"AK8 $m_{SD}^{PUPPI}$ (GeV)",23,40,201),
                            #, weight="weight",
                            )
...
h_mu_muCR4_N2 = hist.Hist("Events / bin",
                          hist.Cat("dataset","Dataset"),
...
h_msd_vs_pt_ak8_muCR4_N2 = hist.Hist("Events / bin",
                                     hist.Cat("dataset","Dataset"),
                                     hist.Cat("passfail","Pass or Fail selection"),
                                     hist.Cat("systematic","Systematic Variations"),
                                     hist.Bin("msd",r"AK8 m_{SD}^{PUPPI} (GeV)",23,40,201),
...
#extras
h_Cuts = hist.Hist("Events / bin",
                   hist.Cat("dataset","Dataset"),
                   hist.Bin("cut",r"$Cut_{i}$",8,0,8))
h_n_ak4 = hist.Hist("Events / bin",
                    hist.Cat("dataset","Dataset"),
                    hist.Bin("n",r"AK4 $n_{jets}$, $p_{T}$ > 30 GeV",20,0,20))
h_ht = hist.Hist("Events / bin",
                 hist.Cat("dataset","Dataset"),
                 hist.Bin("ht","HT (GeV)",50,300,2100),weight="weight")
...
h_n_ak4fwd = hist.Hist("Events / bin",
                       hist.Cat("dataset","Dataset"),
                       hist.Bin("n",r"AK4 $n_{jets}, p_{T} > 30 GeV, 2.5<|\eta|<4.5$",20,0,20))
h_n_ak4L = hist.Hist("Events / bin",
                     hist.Cat("dataset","Dataset"),
                     hist.Bin("n",r"AK4 $n_{L b-tags}, \DeltaR > 0.8, p_{T} > 40 GeV$",20,0,20))
h_n_ak4M = hist.Hist("Events / bin",
                     hist.Cat("dataset","Dataset"),
                     hist.Bin("n",r"AK4 $n_{M b-tags}, \DeltaR > 0.8, p_{T} > 40 GeV$",20,0,20))
h_n_ak4T = hist.Hist("Events / bin",
                     hist.Cat("dataset","Dataset"),
                     hist.Bin("n",r"AK4 $n_{T b-tags}, \DeltaR > 0.8, p_{T} > 40 GeV$",20,0,20))
h_n_ak4dR0p8 = hist.Hist("Events / bin",
                         hist.Cat("dataset","Dataset"),
                         hist.Bin("n",r"AK4 $n_{jets}, \DeltaR > 0.8, p_{T} > 30 GeV$",20,0,20))
h_isolationCA15 = hist.Hist("Events / bin",
                            hist.Cat("dataset","Dataset"),
                            hist.Bin("iso",r"AK8/CA15 $p_{T}$ ratio",50,0.5,1.5))
h_met = hist.Hist("Events / bins",
                  hist.Cat("dataset","Dataset"),
                  hist.Bin("met",r"$E_{T}^{miss}$ (GeV)",50,0,500))
...
h_Mqq = hist.Hist("Events / bin",
                  hist.Cat("dataset","Dataset"),
                  hist.Bin("mqq","Dijet Mass (GeV)",50,0,3000))
h_QGLR = hist.Hist("Events / bin",
                   hist.Cat("dataset","Dataset"),
                   hist.Bin("gqlr","QGLR",10,0,1))
h_Deta_qq = hist.Hist("Events / bin",
                      hist.Cat("dataset","Dataset"),
                      hist.Bin("deta","max Deta qq",30,0,10))
h_t21_ak8 = hist.Hist("Events / bin",
                      hist.Cat("dataset","Dataset"),
                      hist.Bin("tau21",r"AK8 $\tau_{21}$",25,0,1.5),
                      hist.Bin("tau21ddt",r"AK8 $\tau_{21}^{DDT}$",25,0,1.5)
                      )
h_t32_ak8 = hist.Hist("Events / bin",
                      hist.Cat("dataset","Dataset"),
                      hist.Bin("tau32",r"AK8 $\tau_{32}$",20,0,1.5))
h_t32_ak8_t21ddtcut = hist.Hist("Events / bin",
                                hist.Cat("dataset","Dataset"),
                                hist.Bin("tau32",r"AK8 $\tau_{32}$",20,0,1.5))
h_n2b1sd_ak8 =  hist.Hist("Events / bin",
                          hist.Cat("dataset","Dataset"),
                          hist.Bin("N2",r"AK8 $N_{2}^{1}$ (SD)", 25, -0.5, 0.5))
h_n2b1sdddt_ak8 =  hist.Hist("Events / bin",
                             hist.Cat("dataset","Dataset"),
                             hist.Bin("N2",r"AK8 $N_{2}^{1,DDT}$ (SD)", 25, -0.5, 0.5))
h_n2b1sdddt_ak8_aftercut =  hist.Hist("Events / bin",
                                      hist.Cat("dataset","Dataset"),
                                      hist.Bin("N2",r"AK8 $N_{2}^{1}$ (SD)", 25, -0.5, 0.5))
h_dbtag_ak8_aftercut = hist.Hist("Events / bin",
                                 hist.Cat("dataset","Dataset"),
                                 hist.Bin("dbtag",r"$p_{T}$-leading double-b tagger",33,-1,1))

h_msd_ak8_raw_SR = hist.Hist("Events / bin",
                             hist.Cat("dataset","Dataset"),
                             hist.Cat("passfail","pass or fail"),
...
h_msd_ak8_muCR = hist.Hist("Events / bin",
                           hist.Cat("dataset","Dataset"),
                           hist.Cat("region","Analysis Region"),
                           hist.Cat("passfail","pass, fail, none"),
                           hist.Bin("msd",r"AK8 $m_{SD}^{PUPPI}$ (GeV)",23,40,201)
                           )
# more muCR4 breakdown plots
h_pt_mu_muCR4 = hist.Hist("Events / bin",
                          hist.Cat("dataset","Dataset"),
                          hist.Bin("pt",r"leading muon $p_{T}$ (GeV)",50,30,500))
h_eta_mu_muCR4 = hist.Hist("Events / bin",
                           hist.Cat("dataset","Dataset"),
                           hist.Bin("eta",r"leading muon $\eta$",50,-2.5,2.5))
h_pteta_ak8_muCR4 = hist.Hist("Events / bin",
                              hist.Cat("dataset","Dataset"),
                              hist.Bin("pt",r"AK8 leading $p_{T} (GeV)",50,300,2100),
                              hist.Bin("eta",r"AK8 leading \eta",50,-3,3))
h_t21ddt_ak8_muCR4 = hist.Hist("Events / bin",
                               hist.Cat("dataset","Dataset"),
                               hist.Bin("tau21ddt",r"AK8 $#tau_{21}^{DDT}$",25,0,1.5))
h_dbtag_ak8_muCR4 = hist.Hist("Events / bin",
                              hist.Cat("dataset","Dataset"),
